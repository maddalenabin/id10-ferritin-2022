...
def convert2hdf5(xana, filename):
    ...
    with h5py.File( path + filename, 'a') as f: # w for read/write/create access
        ...
        print("Starting with reps")
        for j,rep in enumerate(reps):
            print("\trepetition: ", rep, '\t number of spots: ', len(saxs_indices))
            ...
            I_rep_filtered[j] = np.average(Is, axis=0)
            dI_rep_filtered[j] = np.std(Is, axis=0) / len(ind_saxs)
...
if __name__ == "__main__":
    ...
    ana_db_files = glob(f'/cfs/data/pg/sdaqs/esrf-ebs/id10/sc5275/20220614/processed/results_saxs/{datafolder}_{datasetnumber:04}/p**/analysis_database.pkl', recursive=True)
